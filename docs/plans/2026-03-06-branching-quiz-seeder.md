# Branching Quiz Seeder Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a new quiz seeder with 3 scenario paths (I Know My Peptides, I Know My Goal, I Want Something New) that branch from the first slide using `skip_to_question` and `show_conditions`.

**Architecture:** Follow the `QuizFunnelSeeder` pattern exactly — `slide()` helper with labels, `cond()`/`condAnd()` helpers for conditions, `linkSkipToReferences()` for skip_to wiring, and reusable option helper methods. Each path is self-contained with its own slides and results. The fork slide uses `skip_to_question` on each option to jump to the path's first slide.

**Tech Stack:** Laravel Seeder, QuizQuestion model, QuizFunnelEngine (existing branching infrastructure)

---

### Task 1: Create the seeder file with quiz creation and helpers

**Files:**
- Create: `database/seeders/PeptideFinderProSeeder.php`

**Step 1: Create the seeder skeleton with quiz creation, slide/condition helpers, and reusable option methods**
...
Run: `php artisan list --format=text 2>&1 | head -1`
Expected: No PHP syntax errors

**Step 3: Commit**

```bash
git add database/seeders/PeptideFinderProSeeder.php
git commit -m "feat(quiz): scaffold PeptideFinderPro seeder with helpers"
```

---

### Task 2: Add the fork slide (Slide 1)

**Files:**
- Modify: `database/seeders/PeptideFinderProSeeder.php`

**Step 1: Add the `seedForkSlide()` method**

Add after the helper methods section:
...
{
    // Slide 1: "What brings you here?"
    // skip_to_question values are set later in linkSkipToReferences()
...
**Step 2: Add the `linkSkipToReferences()` method**

```php
// ─── LINK SKIP_TO REFERENCES ──────────────────────────────────────
...
Run: `php -l database/seeders/PeptideFinderProSeeder.php`
...
```bash
git add database/seeders/PeptideFinderProSeeder.php
git commit -m "feat(quiz): add fork slide with 3-path branching"
```

---

### Task 3: Add Path 1 — "I Know My Peptides" (5 slides)

**Files:**
- Modify: `database/seeders/PeptideFinderProSeeder.php`

**Step 1: Add the `seedPath1KnowMyPeptides()` method**
...
private function seedPath1KnowMyPeptides(Quiz $quiz): void
{
    ...
    // P1-2: Intermission — peptide summary
    ...
    // P1-4: Vendor reveal — shows deals for the selected peptide
...
```bash
git add database/seeders/PeptideFinderProSeeder.php
git commit -m "feat(quiz): add Path 1 — I Know My Peptides (5 slides)"
```

---

### Task 4: Add Path 2 — "I Know My Goal" (~25 slides)

This is the longest path, carrying over the core profiling questions from the existing PeptideFinderSeeder.

**Files:**
- Modify: `database/seeders/PeptideFinderProSeeder.php`

**Step 1: Add the `seedPath2KnowMyGoal()` method**
...
private function seedPath2KnowMyGoal(Quiz $quiz): void
{
    ...
    // P2-2: Intermission — "You're Not Alone"
    ...
    // P2-21: Peptide Reveal (uses ResultsBank multi-peptide resolution)
    ...
    // P2-22: Vendor Reveal (shows stores for all matched peptides)
...
```bash
git add database/seeders/PeptideFinderProSeeder.php
git commit -m "feat(quiz): add Path 2 — I Know My Goal (23 slides with profiling)"
```

---

### Task 5: Add Path 3 — "I Want Something New" (10 slides)

**Files:**
- Modify: `database/seeders/PeptideFinderProSeeder.php`

**Step 1: Add the `seedPath3WantSomethingNew()` method**
...
private function seedPath3WantSomethingNew(Quiz $quiz): void
{
    ...
    // P3-5: Intermission — encouragement
    ...
    // P3-8: Peptide Reveal (excludes their previous peptide via exclusion logic)
...
```bash
git add database/seeders/PeptideFinderProSeeder.php
git commit -m "feat(quiz): add Path 3 — I Want Something New (10 slides)"
```

---

### Task 6: Add outcomes and run the seeder

**Files:**
- Modify: `database/seeders/PeptideFinderProSeeder.php`

**Step 1: Add the `seedOutcomes()` method**
...
Run: `php artisan db:seed --class=PeptideFinderProSeeder`
Expected: `Seeded 39 Peptide Finder Pro slides across 3 paths.` and `Seeded 4 quiz outcomes.`

**Step 3: Verify the quiz exists and has correct slide count**

Run: `php artisan tinker --execute='$q = App\Models\Quiz::where("slug","peptide-finder-pro")->first(); echo "Quiz: ".$q->name." | Slides: ".$q->questions()->count()." | Outcomes: ".$q->outcomes()->count();'`
Expected: `Quiz: Peptide Finder Pro | Slides: 39 | Outcomes: 4`

**Step 4: Verify the fork slide has skip_to_question set**

Run: `php artisan tinker --execute='$q = App\Models\Quiz::where("slug","peptide-finder-pro")->first(); $fork = $q->questions()->where("order",1)->first(); foreach($fork->options as $o) echo $o["value"]." => skip_to: ".($o["skip_to_question"] ?? "null")."\n";'`
Expected: Each option shows a non-null skip_to_question ID

**Step 5: Verify show_conditions on Path 3's conditional slide (p3_new_goal)**

Run: `php artisan tinker --execute='$q = App\Models\Quiz::where("slug","peptide-finder-pro")->first(); $slide = $q->questions()->whereJsonContains("show_conditions->conditions", ["option_value" => "different_category"])->first(); echo $slide ? "Found: ".$slide->question_text : "NOT FOUND";'`
Expected: `Found: What health goal do you want to explore?`

**Step 6: Commit**

```bash
git add database/seeders/PeptideFinderProSeeder.php
git commit -m "feat(quiz): add outcomes and complete PeptideFinderPro seeder"
```

---

### Task 7: Register seeder in DatabaseSeeder and browser test

**Files:**
- Modify: `database/seeders/DatabaseSeeder.php` (add call to PeptideFinderProSeeder)

**Step 1: Check if DatabaseSeeder references seeders, and add the new one if appropriate**

Run: Read `database/seeders/DatabaseSeeder.php` and add:

```php
$this->call(PeptideFinderProSeeder::class);
```

Only if other quiz seeders are called there. If not, skip this — the seeder can be run directly.

**Step 2: Browser test the quiz**

Navigate to: `http://pepprofesor.test/quiz/peptide-finder-pro`

**Test Path 1 (I Know My Peptides):**
1. Select "I know my peptides"
2. Verify: jumps to peptide selector (skip_to works)
3. Select any peptide (e.g., BPC-157)
4. Verify: intermission → email capture → vendor reveal → bridge
5. Verify: only ~5 slides total, vendor reveal shows deals for BPC-157

**Test Path 2 (I Know My Goal):**
1. Restart quiz, select "I know my goal"
2. Verify: jumps to health goal picker
3. Select "Lose fat & boost metabolism"
4. Progress through profiling questions (gender, age, health, exercise, etc.)
5. Verify: ~23 slides total, peptide reveal shows Tirzepatide + alternatives, vendor reveal shows all matched stores

**Test Path 3 (I Want Something New):**
1. Restart quiz, select "I want something new"
2. Verify: jumps to "What peptide were you using?"
3. Select BPC-157, pick "Not seeing results", pick "Stay in same category"
4. Verify: slide 4 (health goal picker) is SKIPPED (because "same_category" selected)
5. Verify: recommendation excludes BPC-157, shows alternatives for injury_recovery
6. Restart and test "Different category" → verify health goal picker appears

**Step 3: Fix any issues found during testing**

```bash
git add -A
git commit -m "fix(quiz): adjustments from browser testing PeptideFinderPro"
```

---

### Task 8: Handle Path 3 "same_category" health goal resolution

**Context:** When Path 3 user selects "Stay in same category", there's no `health_goal` answer set — the QuizPlayer needs to infer the health goal from the peptide they were using. This requires a small mapping in QuizPlayer.

**Files:**
- Modify: `app/Livewire/QuizPlayer.php`

**Step 1: Add a peptide-to-category mapping method**

Add after the existing `getAnswerByKlaviyoProperty` method:

```php
/**
 * Infer health goal from a peptide slug (for Path 3 "same category" logic).
 * Falls back to looking up the peptide in ResultsBank.
 */
private function inferHealthGoalFromPeptide(string $peptideSlug): ?string
{
    $entry = \App\Models\ResultsBank::where('peptide_slug', $peptideSlug)
...
**Step 2: Update `getResultsBankEntryProperty` to use inferred goal when health_goal is not set**

In the existing `getResultsBankEntryProperty()` method (around line 173), update the health goal resolution:
...
Apply the same change to `getResultsBankEntriesProperty()` if it exists (from the multi-peptide plan).
...
**Step 4: Browser test Path 3 "same category"**

1. Take the quiz, select "I want something new"
2. Select BPC-157 (injury_recovery category)
3. Select any switch reason
4. Select "Stay in same category"
5. Verify: peptide reveal shows injury_recovery alternatives (excluding BPC-157)

**Step 5: Commit**

```bash
git add app/Livewire/QuizPlayer.php
git commit -m "feat(quiz-player): infer health goal from peptide for Path 3 same-category logic"
```
...
1. Existing quiz (`peptide-finder-100`) still works — no changes made to it
2. New quiz (`peptide-finder-pro`) accessible at `/quiz/peptide-finder-pro`
3. Path 1: 5 slides, shows vendor deals for selected peptide
4. Path 2: ~23 slides, full profiling → recommendation → vendor comparison
5. Path 3: ~10 slides, excludes previous peptide, respects same/different category choice
6. Fork slide correctly jumps to each path via `skip_to_question`
7. `show_conditions` correctly filters slides per path (no cross-path bleed)
8. Path 3's conditional health goal picker only shows when "Different category" is selected
9. All Klaviyo properties are tracked correctly per path
10. Outcomes resolve correctly per path
