        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            @foreach($categories as $category)
                <a href="{{ route('peptides.index', ['category' => $category->slug]) }}"
                   class="group relative bg-surface-50 rounded-2xl p-5 hover:bg-surface-100 border border-surface-200 hover:border-primary-300 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                    {{-- Icon --}}
                    <div class="w-10 h-10 rounded-xl bg-primary-100 flex items-center justify-center mb-3 group-hover:bg-primary-200 group-hover:scale-110 transition-all">
