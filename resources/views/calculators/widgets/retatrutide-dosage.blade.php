{{-- Retatrutide dosage widget — reconstitution + weekly mg → units + reference ladder --}}
<div x-data="retatrutideCalc()" class="space-y-6">
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="grid md:grid-cols-2">
            {{-- Inputs --}}
            <div class="p-6 sm:p-8 space-y-5">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Retatrutide in vial (mg)</label>
                    <input type="number" min="0" step="1" x-model.number="mg" class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" min="0" step="0.5" x-model.number="dose" class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500">
                </div>
            </div>
            {{-- Result --}}
    ...
    {{-- Reference ladder --}}
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <h3 class="font-bold text-gray-900 px-6 pt-5">Dose reference — units for your reconstitution</h3>
        <p class="text-sm text-gray-500 px-6 pb-3">A typical escalating range converted for your vial. Follow your specific protocol.</p>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody>
                    <template x-for="d in ladder" :key="d">
...
<script>
    function retatrutideCalc() {
        const defaults = { mg: 10, water: 2, dose: 2 };
        return {
            ...defaults, copied: false,
            ladder: [1, 2, 4, 6, 8, 12],
            reset() { Object.assign(this, defaults); },
            copy() { try { navigator.clipboard.writeText(`${this.dose} mg retatrutide = ${this.units(this.dose)} units (${this.ml(this.dose)} mL)`); } catch (e) {} this.copied = true; setTimeout(() => this.copied = false, 1500); },
