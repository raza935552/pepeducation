<x-account-layout>
    <div class="space-y-6">
        @if(session('success'))
            <div class="p-4 rounded-lg bg-green-50 text-green-700 text-sm border border-green-200">
                {{ session('success') }}
            </div>
        @endif

        {{-- Profile Form --}}
        <form action="{{ route('account.profile.update') }}" method="POST" class="bg-white rounded-xl shadow-sm border border-surface-200 p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}"
                           class="w-full rounded-lg border-surface-200 focus:ring-primary-500 focus:border-primary-500">
                    @error('name')
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input type="email" value="{{ $user->email }}" disabled
                           class="flex-1 rounded-lg border-surface-200 bg-surface-50 cursor-not-allowed">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-surface-200 p-6">
            ...
            <dl class="grid sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <dd class="text-sm font-medium text-gray-900">{{ $user->created_at->format('F j, Y') }}</dd>
                </div>
                <div>
