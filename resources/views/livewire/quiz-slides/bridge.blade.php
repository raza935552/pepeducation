...
<div class="card p-8 text-center" wire:key="slide-bridge-{{ $currentStep }}">
    ...
    <div class="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-6">
        <svg aria-hidden="true" class="w-8 h-8 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @else
        <button wire:click="advanceSlide" wire:loading.attr="disabled" wire:loading.class="opacity-50 pointer-events-none" class="btn btn-primary text-lg px-8 py-3">Continue</button>
    @endif

    {{-- Finish Quiz button — always present to complete the quiz --}}
    <div class="mt-4">
        <button wire:click="advanceSlide" wire:loading.attr="disabled" wire:loading.class="opacity-50 pointer-events-none" class="text-sm font-medium text-primary hover:text-primary/80 underline underline-offset-2">
