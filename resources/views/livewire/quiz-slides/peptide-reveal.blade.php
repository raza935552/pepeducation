...
<div class="card p-8" wire:key="slide-peptide-reveal-{{ $currentStep }}">
    ...
    @if($result)
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-sm text-primary font-semibold uppercase tracking-wide mb-2">{{ $settings['pre_headline'] ?? 'Your Personalized Recommendation' }}</p>
    ...
    {{-- Per-product accordion from ResultsBank (takes priority), then slide-level fallback --}}
    @php
        $rbAccordionItems = $result?->accordion_items ?? [];
    @endphp
    @if(!empty($rbAccordionItems))
        <div class="mt-6" x-data="{ openItem: null }">
            <div class="space-y-2">
                @foreach($rbAccordionItems as $index => $item)
