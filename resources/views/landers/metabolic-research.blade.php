<x-public-layout title="Where Metabolic Research Is Going Next | Biolinx Labs" description="GLP-1 was just the beginning. A look at where metabolic research is heading next.">
@push('head')
<style>
...
(function(){
  ...
  window.lmSubmit=function(ev){
    ...
    try{ fetch('/subscriber/sync',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':csrf,'Accept':'application/json'},credentials:'same-origin',body:JSON.stringify({email:em.value,source:'lp-metabolic-research'})}).then(done).catch(done); }catch(e){done();}
    return false;
  };
