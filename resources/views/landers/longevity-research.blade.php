<x-public-layout title="Studying Aging at the Cellular Level | Biolinx Labs" description="Beyond the anti-aging headlines: how researchers study aging at the cellular level.">
@push('head')
<style>
...
(function(){
  ...
  window.lmSubmit=function(ev){
    ...
    try{ fetch('/subscriber/sync',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':csrf,'Accept':'application/json'},credentials:'same-origin',body:JSON.stringify({email:em.value,source:'lp-longevity-research'})}).then(done).catch(done); }catch(e){done();}
    return false;
  };
