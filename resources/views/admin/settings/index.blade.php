<x-admin-layout>
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST" class="space-y-6">
        ...
        <div class="card p-6 border-l-4 border-green-400">
            ...
            <p class="text-sm text-gray-500 mb-4">Sync quiz subscribers and events to Customer.io for messaging automation.</p>
            @php $cioSettings = \App\Models\CustomerIoSetting::current(); @endphp
            <div class="flex items-center gap-4">
                <a href="{{ route('admin.settings.customerio') }}" class="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-medium">Configure Customer.io</a>
                @if($cioSettings && $cioSettings->is_enabled)
                    <span class="inline-flex items-center px-2 py-1 text-xs font-medium bg-green-100 text-green-700 rounded-full">Active</span>
                @else
                    <span class="inline-flex items-center px-2 py-1 text-xs font-medium bg-gray-100 text-gray-500 rounded-full">Disabled</span>
                @endif
            </div>
        </div>
