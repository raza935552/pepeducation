    ...
    <div class="max-w-2xl space-y-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <form action="{{ route('admin.subscribers.destroy', $subscriber) }}" method="POST"
                ...
                @method('DELETE')
                <button type="submit"
                        class="px-4 py-2 text-red-600 hover:text-red-700 border border-red-300 rounded-lg hover:bg-red-50 transition-colors">
                    Delete Subscriber
                </button>
            </form>
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-gray-900">{{ $subscriber->email }}</h2>
            ...
            <div class="px-6 py-4 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-500 uppercase">Subscribed At</label>
                        <p class="mt-1 text-gray-900">{{ $subscriber->subscribed_at->format('F j, Y g:i A') }}</p>
                    </div>
                </div>

                @if($subscriber->unsubscribed_at)
                    <div>
                        <label class="block text-xs font-medium text-gray-500 uppercase">Unsubscribed At</label>
                        <p class="mt-1 text-gray-900">{{ $subscriber->unsubscribed_at->format('F j, Y g:i A') }}</p>
                    </div>
                @endif
                ...
                @if($subscriber->ip_address)
                    <div>
                        ...
                        <p class="mt-1 text-gray-900 font-mono text-sm">{{ $subscriber->ip_address }}</p>
                    </div>
                @endif
                ...
                @if($subscriber->user_agent)
                    <div>
                        ...
                        <p class="mt-1 text-gray-600 text-sm break-all">{{ $subscriber->user_agent }}</p>
                    </div>
                @endif
                ...
                <div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                    <div>
                        ...
                        <p class="mt-1 text-gray-600 text-sm">{{ $subscriber->created_at->format('M j, Y') }}</p>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-500 uppercase">Last Updated</label>
                        <p class="mt-1 text-gray-600 text-sm">{{ $subscriber->updated_at->format('M j, Y') }}</p>
                    </div>
                </div>
            </div>
        </div>
