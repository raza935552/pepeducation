<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
    <div class="px-4 py-3 border-b border-gray-200">
        <h3 class="font-semibold text-gray-900">First Touch Attribution</h3>
    </div>
    <div class="p-4 space-y-3 text-sm">
        <div class="flex justify-between">
            <span class="text-gray-500">Source</span>
            <span class="text-gray-900 font-medium">{{ $subscriber->source ?? 'Unknown' }}</span>
        </div>
        @if($subscriber->first_landing_page)
            <div class="flex justify-between">
                <span class="text-gray-500">Landing Page</span>
                <span class="text-gray-900 font-medium truncate max-w-[200px]" title="{{ $subscriber->first_landing_page }}">
        ...
        @if($subscriber->first_utm_source)
            <div class="flex justify-between">
                ...
                <span class="text-gray-900 font-medium">{{ $subscriber->first_utm_source }}</span>
            </div>
        @endif
        @if($subscriber->first_utm_medium)
            <div class="flex justify-between">
                <span class="text-gray-500">UTM Medium</span>
                <span class="text-gray-900 font-medium">{{ $subscriber->first_utm_medium }}</span>
            </div>
        @endif
        @if($subscriber->first_utm_campaign)
            <div class="flex justify-between">
                <span class="text-gray-500">UTM Campaign</span>
                <span class="text-gray-900 font-medium">{{ $subscriber->first_utm_campaign }}</span>
            </div>
        @endif
        @if($subscriber->first_referrer)
            <div class="flex justify-between">
                <span class="text-gray-500">Referrer</span>
                <span class="text-gray-900 font-medium truncate max-w-[200px]">{{ $subscriber->first_referrer }}</span>
            </div>
        @endif
        @if($subscriber->first_session_id)
            <div class="flex justify-between">
                <span class="text-gray-500">Session ID</span>
                <span class="text-gray-600 font-mono text-xs">{{ Str::limit($subscriber->first_session_id, 20) }}</span>
            </div>
        @endif
