<x-admin-layout>
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Peptide Requests</h1>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('admin.requests.index', ['status' => 'in_progress']) }}"
               class="px-4 py-2 rounded-lg text-sm font-medium {{ request('status') === 'in_progress' ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
                In Progress ({{ $counts['in_progress'] }})
            </a>
            <a href="{{ route('admin.requests.index', ['status' => 'published']) }}"
               class="px-4 py-2 rounded-lg text-sm font-medium {{ request('status') === 'published' ? 'bg-green-500 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600' }}">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow overflow-hidden">
            @if($requests->count() > 0)
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Peptide Name</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Requested By</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Sources</th>
                        ...
                        @foreach($requests as $request)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="font-medium text-gray-900 dark:text-white">{{ $request->peptide_name }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($request->user)
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="mt-2 text-gray-500 dark:text-gray-400">Peptide requests will appear here.</p>
                </div>
            @endif
        </div>
    </div>
