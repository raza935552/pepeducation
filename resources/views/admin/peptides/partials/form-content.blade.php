<div class="card">
    ...
    <!-- Mechanism of Action -->
    <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 mb-1">Mechanism of Action</label>
    ...
    <div class="mb-4" x-data="{ items: {{ json_encode(old('key_benefits', $peptide?->key_benefits ?? [])) }} }">
        <label class="block text-sm font-medium text-gray-700 mb-1">Key Benefits</label>
    ...
    <div class="mb-4" x-data="{ items: {{ json_encode(old('what_to_expect', $peptide?->what_to_expect ?? [])) }} }">
        <label class="block text-sm font-medium text-gray-700 mb-1">What to Expect (Timeline)</label>
        <div class="space-y-2 mb-2">
            <template x-for="(item, index) in items" :key="index">
                <div class="flex gap-2">
    ...
    <div x-data="{ items: {{ json_encode(old('safety_warnings', $peptide?->safety_warnings ?? [])) }} }">
        <label class="block text-sm font-medium text-gray-700 mb-1">Safety Warnings</label>
        <div class="space-y-2 mb-2">
            <template x-for="(item, index) in items" :key="index">
                <div class="flex gap-2">
