<x-admin-layout>
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <a href="{{ route('admin.messages.index') }}" class="text-sm text-gray-500 hover:text-gold-500 flex items-center gap-1">
                    <svg aria-hidden="true" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                    </svg>
                    Back to Messages
                </a>
                <h1 class="text-2xl font-bold text-gray-900 mt-2">{{ $message->subject }}</h1>
            </div>
        ...
        <div class="grid lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-xl shadow p-6">
                    <div class="flex items-start gap-4">
                        <div class="w-12 h-12 rounded-full bg-gold-100 flex items-center justify-center flex-shrink-0">
                            <span class="text-lg font-bold text-gold-600">{{ strtoupper(substr($message->name, 0, 1)) }}</span>
                        </div>
                ...
                @if($message->admin_notes)
                    <div class="bg-yellow-50 rounded-xl p-6 border border-yellow-200">
                        ...
                        <p class="text-gray-700">{{ $message->admin_notes }}</p>
                    </div>
                @endif
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow p-6">
                    ...
                    <form action="{{ route('admin.messages.status', $message) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="admin_notes" rows="3" class="w-full rounded-lg border-gray-300 focus:ring-gold-500 focus:border-gold-500">{{ $message->admin_notes }}</textarea>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow p-6">
                    ...
                    <div class="space-y-3">
                        <a href="mailto:{{ $message->email }}" class="flex items-center gap-2 text-gray-700 hover:text-gold-500">
