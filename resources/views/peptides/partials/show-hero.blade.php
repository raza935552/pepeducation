    ...
    <div class="flex-1">
        ...
        <div class="flex flex-wrap items-center gap-3 mb-3">
            @if($peptide->abbreviation)
                <span class="px-3 py-1 rounded-full bg-white/10 text-sm font-mono text-primary-300">
                    {{ $peptide->abbreviation }}
                </span>
            @endif
            <span class="text-sm text-surface-400">{{ $peptide->type }}</span>
        </div>

        <h1 class="text-3xl md:text-4xl lg:text-5xl font-bold mb-2 bg-gradient-to-r from-white to-surface-200 bg-clip-text text-transparent">
            {{ $peptide->name }}
        </h1>
    ...
    <div class="shrink-0 w-full lg:w-auto space-y-3">
        <div class="flex gap-2">
            ...
            <form action="{{ route('bookmarks.toggle', $peptide) }}" method="POST">
                @csrf
                <button type="submit" class="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl transition-all {{ $isBookmarked ? 'bg-primary-500 text-white hover:bg-primary-600' : 'bg-white/10 text-surface-200 hover:bg-white/20 border border-white/10' }}">
        ...
        @else
            <a href="{{ route('login') }}" class="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 text-surface-200 hover:bg-white/20 border border-white/10 transition-all">
        ...
        @endauth
            ...
            <button type="button"
                    ...
                    class="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 text-surface-200 hover:bg-white/20 border border-white/10 transition-all"
        ...
        <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/10 p-4 lg:p-6 lg:min-w-[200px]">
            ...
            <div class="relative flex lg:block items-center justify-between gap-4">
                <div>
                    <div class="text-xs uppercase tracking-wider text-surface-400 mb-1 lg:mb-2">Research Status</div>
                    <div class="text-xl lg:text-2xl font-bold
                        {{ $badge['color'] === 'blue' ? 'text-blue-400' : '' }}
                        {{ $badge['color'] === 'green' ? 'text-emerald-400' : '' }}
                        {{ $badge['color'] === 'yellow' ? 'text-primary-400' : '' }}
                        {{ $badge['color'] === 'gray' ? 'text-surface-400' : '' }}">
                        {{ $badge['label'] }}
                    </div>
                </div>
                <div class="flex gap-1 lg:mt-3">
                    @for($i = 0; $i < 4; $i++)
                        <div class="w-6 lg:w-8 h-1.5 rounded-full {{ $i < ($badge['color'] === 'blue' ? 4 : ($badge['color'] === 'green' ? 3 : ($badge['color'] === 'yellow' ? 2 : 1))) ? ($badge['color'] === 'yellow' ? 'bg-primary-400' : 'bg-'.$badge['color'].'-400') : 'bg-dark-700' }}"></div>
                    @endfor
                </div>
            </div>
        </div>
