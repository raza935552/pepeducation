@if($peptides->isEmpty())
    <div class="bg-white rounded-2xl border border-surface-200 p-12 text-center">
        <div class="w-20 h-20 mx-auto mb-6 rounded-2xl bg-surface-100 flex items-center justify-center">
            <svg aria-hidden="true" class="w-10 h-10 text-surface-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mt-10 flex justify-center">
            <nav class="flex items-center gap-2" role="navigation" aria-label="Pagination">
                ...
                @if($peptides->onFirstPage())
                    <span class="px-4 py-2 rounded-xl bg-surface-100 text-surface-400 cursor-not-allowed">
                        <svg aria-hidden="true" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @else
                    <a href="{{ $peptides->previousPageUrl() }}"
                       class="px-4 py-2 rounded-xl bg-white border border-surface-200 text-gray-600 hover:bg-primary-50 hover:border-primary-300 transition-colors">
                        <svg aria-hidden="true" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                        </svg>
                    </a>
                ...
                @foreach($peptides->getUrlRange(max(1, $peptides->currentPage() - 2), min($peptides->lastPage(), $peptides->currentPage() + 2)) as $page => $url)
                    @if($page == $peptides->currentPage())
                        <span class="px-4 py-2 rounded-xl bg-primary-500 text-white font-medium min-w-[44px] text-center">
                            {{ $page }}
                        </span>
                    @else
                        <a href="{{ $url }}"
                           class="px-4 py-2 rounded-xl bg-white border border-surface-200 text-gray-600 hover:bg-primary-50 hover:border-primary-300 transition-colors min-w-[44px] text-center">
                            {{ $page }}
                        </a>
                    @endif
                @endforeach
