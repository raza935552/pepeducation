    ...
    <div class="card" x-data="{ showAll: false }">
        ...
        <div class="space-y-1">
            <a href="{{ route('peptides.index', array_filter(['search' => request('search'), 'research' => request('research')])) }}"
               class="block px-3 py-2 rounded-lg text-sm transition-colors {{ !request('category') ? 'bg-primary-100 text-primary-700' : 'text-gray-600 hover:bg-surface-100' }}">
                All Categories
            </a>
            @foreach($categories->take(8) as $cat)
                <a href="{{ route('peptides.index', array_filter(['category' => $cat->slug, 'search' => request('search'), 'research' => request('research')])) }}"
                   class="flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors {{ request('category') === $cat->slug ? 'bg-primary-100 text-primary-700' : 'text-gray-600 hover:bg-surface-100' }}">
                    <span class="flex items-center gap-2">
                        <span class="w-2 h-2 rounded-full" style="background-color: {{ $cat->color }}"></span>
                        {{ $cat->name }}
                    </span>
                    <span class="text-xs text-surface-500">{{ $cat->peptides_count }}</span>
                </a>
            @endforeach
            ...
            @if($categories->count() > 8)
                ...
                <button @click="showAll = !showAll" class="w-full text-left px-3 py-2 text-sm text-primary-600 hover:underline">
        ...
        <div class="space-y-1">
            @foreach(['extensive' => 'Extensively Studied', 'well' => 'Well Researched', 'emerging' => 'Emerging', 'limited' => 'Limited'] as $val => $label)
                <a href="{{ route('peptides.index', array_filter(['research' => request('research') === $val ? null : $val, 'category' => request('category'), 'search' => request('search')])) }}"
                   class="block px-3 py-2 rounded-lg text-sm transition-colors {{ request('research') === $val ? 'bg-primary-100 text-primary-700' : 'text-gray-600 hover:bg-surface-100' }}">
                    {{ $label }}
                </a>
            @endforeach
        </div>
