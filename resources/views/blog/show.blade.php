                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        @foreach($post->categories as $category)
                            <a href="{{ route('blog.category', $category) }}"
                               class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-white/10 text-surface-200 hover:bg-white/20 transition-colors">
                                {{ $category->name }}
                            </a>
                        @endforeach
                    </div>
                        ...
                        <div class="flex flex-wrap gap-3">
                            @foreach($post->peptides as $peptide)
                                <a href="{{ route('peptides.show', $peptide) }}"
                                   class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-surface-50 border border-surface-200 text-dark-700 hover:bg-surface-100 transition-colors">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @foreach($post->tags as $tag)
                                <a href="{{ route('blog.tag', $tag) }}"
                                   class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-surface-100 text-dark-700 hover:bg-surface-200 transition-colors">
                                    #{{ $tag->name }}
                                </a>
                            @endforeach
                        </div>
